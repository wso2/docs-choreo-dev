...
i.e. The `base` in `prod` will generally be in synch with a tag of the master's `base`.
...
All changes have to first be tested on the `local` cluster using the latest images. You will generally use the `dev`
 ...
 objects such as deployments, services and so on, first make the change in your `master` branch and then synch that to 
