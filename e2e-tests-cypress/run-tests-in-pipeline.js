...
async function runTests(buildNumber, buildId) {
  cypress
    .run({
      ...
      tag: buildNumber,
      ciBuildId: buildId,
      spec: ['cypress/e2e-stable/devportal/*.ts','cypress/e2e-stable/console/**/*','cypress/e2e-unstable/devportal/*.ts','cypress/e2e-unstable/console/**/*','!cypress/e2e-stable/console/1-component/perf/**/*.ts'],
    })
    .then((results) => {
