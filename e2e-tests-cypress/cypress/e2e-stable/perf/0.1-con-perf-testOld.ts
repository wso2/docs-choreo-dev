    ...
    cy.wait("@createProjectRequest", PERF_INTERCEPT_WAIT_TIME).then(
      (interception) => {
        try {
          ...
          interceptWriter.interceptAndWriteToFixture("createProjectRequest", filePath, interception.request, interception.response);
        }
      }
    );
  });
    ...
    cy.wait("@buildComponentRequest", PERF_INTERCEPT_WAIT_TIME).then(
      (interception) => {
        try {
          ...
          cy.log(`Assertion error: ${error.message}`);
          interceptWriter.interceptAndWriteToFixture("buildComponentRequest",filePath, interception.request, interception.response);
        }
      }
    );
  });
    ...
    cy.wait("@deployComponentRequest", PERF_INTERCEPT_WAIT_TIME).then(
      (interception) => {
        try {
          ...
          cy.log(`Assertion error: ${error.message}`);
          interceptWriter.interceptAndWriteToFixture("deployComponentRequest",filePath, interception.request, interception.response);
        }
      }
    );
  });
