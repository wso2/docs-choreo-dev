...
import { INSIGHTS_URL, PUBLISHER_APIS } from "../../../../commons/urls";

export class _ProxyOverview {
  private menu = new ProxyLeftMenu();

  navigateToDevPortal(component: Proxy, idp: string) {
    cy.intercept({ method: "GET", url: PUBLISHER_APIS, times: 1 }).as("apis");
    cy.intercept({ method: "POST", url: INSIGHTS_URL, times: 1 }).as(
      "insights"
    );

    this.menu.navigateToOverview();

    cy.wait(["@apis", "@insights"], SHORT_TIME).then(() => {
