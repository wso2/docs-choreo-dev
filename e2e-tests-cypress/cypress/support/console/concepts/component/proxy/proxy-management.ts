...
export class _ProxyManagement {
  ...
  applyAllPermissionsToResources(component: Proxy, permissions: string[]) {
  ...
  deleteAllPermissionsFromResources(component: Proxy, permissions: string[]) {
  ...
  applyPermissionToResources(component: Proxy, permission: string) {
  ...
  private publish(component: Proxy) {
    ...
    cy.wait(10000); // Extra wait because the proxy is not updated immediately
  }
  ...
  private toggleCors(component: Proxy, isEnabled: boolean) {
    ProxyUtils.validateDeploymentTrack(component);
