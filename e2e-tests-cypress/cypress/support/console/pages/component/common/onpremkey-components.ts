...
static navigateToOpPremKeySettings(){
  ...
  static generateOnPremKey() {
    ...
    cy.get('[data-testid="on-prem-key-copy-btn"]',{timeout:120000}).should('be.visible');
  ...
  static editOnPremKey() {
    ...
    cy.contains(OnPremkeyComponent.keyName).parent().find('[data-testid="key-edit-btn"]').click();
    cy.get('[data-testid="on-prem-key"]').should('exist');
