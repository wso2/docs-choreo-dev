...
export class DomainsComponents {
    ...
    static deleteDevportalDomainIfExists(domainName: string) {
        cy.get('[data-cyid="devportal-custom-domain-search-field"]').trigger("mouseover");
    ...
    private static deleteSelectedDomain(domainName: string) {
        cy.get('[class="MuiTableCell-root MuiTableCell-body MuiTableCell-alignLeft"]').dblclick();
    ...
    static createDevportalDomain(domainName: string) {
        ...
        cy.get('[data-cyid="btn-add"]').click({ force: true });
        cy.get('[data-cyid="btn-add"]').should("not.exist");
        cy.contains("td", domainName).should("be.visible");
