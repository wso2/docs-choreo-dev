...
public class TestConfigurationGroups extends TestBase {
       ...
       @CitrusTest
       public void createComponent_TestConfigGroupsDp(DataProviderWrapper dp) throws Exception {
               ...
               Repository repo = Repository.builder().repoUrl("https://github.com/choreo-test-apps/configuration-test-repo")
                                .buildContext("configuration-groups-sample").isPublicRepo(false).build();
        ...
        @Test(dependsOnMethods = { "createComponent_TestConfigGroupsDp" }, dataProvider = "dps")
        @CitrusTest
        public void createConfigGroup_TestConfigGroupsDp(DataProviderWrapper dp) throws Exception {
                String configGroupName = NameGenerator.generateUniqueName(Constant.TEST_CONFIG_GROUP_NAME);
                ConfigurationGroup configGroupCreationPayload = TestHelper.generateConfigGroupCreationPayload(
                        configGroupName, dp.getEnvironments());

                configurationGroup = ConfigServiceUtils.createConfigGroup(this,
                        citrusClients.get(Endpoints.CHOREO_NEW_APP_SERVICE_ENDPOINT), configGroupCreationPayload);
        }

        @Test(dependsOnMethods = { "createConfigGroup_TestConfigGroupsDp" }, dataProvider = "dps")
        @CitrusTest
        public void addConfigurations_TestConfigGroupsDp(DataProviderWrapper dp) throws Exception {
            ...
            environments.forEach(environment -> {
                if (environment.getName().equals(Constant.Environment.Development.name())) {
                    environmentTemplateIdRef.set(environment.getTemplateId());
                }
            });
            String envTemplateId = environmentTemplateIdRef.get();
            ConfigManagement.createConfigurationMapping(this,
                    citrusClients.get(Endpoints.CHOREO_NEW_APP_SERVICE_ENDPOINT), dp.getChoreoComponent(),
                    configurationGroup, envTemplateId);
        }

        @Test(dependsOnMethods = { "addConfigurations_TestConfigGroupsDp" }, dataProvider = "dps")
        @CitrusTest
        public void deployComponent_TestConfigGroupsDp(DataProviderWrapper dp) throws Exception {
        ...
        @Test(dependsOnMethods = { "deployComponent_TestConfigGroupsDp" }, dataProvider = "dps")
        @CitrusTest
        public void invokeAPIDev_TestConfigGroupsDp(DataProviderWrapper dp) throws Exception {
            ...
            ComponentUtils.invokeApiGET(this, invokeData.getRight().getApikey(), invokeData.getLeft(),
                    "/greeter/configs",
                    TestHelper.EXPECTED_MAPPING_API_DEV_RESPONSE);
        }

    @Test(dependsOnMethods = { "invokeAPIDev_TestConfigGroupsDp" }, dataProvider = "dps")
    @CitrusTest
    public void promoteComponent_TestConfigGroupsDp(DataProviderWrapper dp) throws Exception {
        AtomicReference<String> environmentTemplateIdRef = new AtomicReference<>(null);
        List<Environment> environments = dp.getEnvironments();
        environments.forEach(environment -> {
    ...
    @Test(dependsOnMethods = { "promoteComponent_TestConfigGroupsDp" }, dataProvider = "dps")
    @CitrusTest
    public void invokeAPIProd_TestConfigGroupsDp(DataProviderWrapper dp) throws Exception {
        for (ComponentDeploymentStatusDTO statusDTO : dp.getPromoteStatusDTO()) {
            ...
            ComponentUtils.invokeApiGET(this, invokeData.getRight().getApikey(), invokeData.getLeft(),
                    "/greeter/configs",
                    TestHelper.EXPECTED_MAPPING_API_PROD_RESPONSE);
        }
    }

    @Test(dependsOnMethods = { "invokeAPIProd_TestConfigGroupsDp" }, dataProvider = "dps")
    @CitrusTest
    public void deleteConfigGroup_TestConfigGroupsDp(DataProviderWrapper dp) throws Exception {
        ConfigServiceUtils.deleteConfigGroup(this, citrusClients.get(Endpoints.CHOREO_NEW_APP_SERVICE_ENDPOINT),
                configurationGroup.getGroupUuid());
    }
}
