...
IF NOT EXISTS
	(SELECT name FROM master.sys.sql_logins
	WHERE name = 'choreo_delete_manager_admin_db_user')
CREATE LOGIN choreo_delete_manager_admin_db_user with password = N'xxxxxxxxxxxxx'
GO
...
IF NOT EXISTS
	(SELECT name FROM master.sys.sql_logins
	WHERE name = 'choreo_cp_ris_db_user')
CREATE LOGIN choreo_cp_ris_db_user with password = N'xxxxxxxxxxxxx'
GO
