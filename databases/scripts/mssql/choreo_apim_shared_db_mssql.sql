-- Create User
IF EXISTS (SELECT name FROM sys.databases WHERE name = N'choreo_apim_shared_db') AND NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = N'choreo_apim_shared_db_user')
BEGIN
    CREATE USER [choreo_apim_shared_db_user] with password = N'${choreo_apim_shared_db_mssql_password}'
    GRANT SELECT, INSERT, UPDATE, DELETE, EXECUTE ON DATABASE::choreo_apim_shared_db TO choreo_apim_shared_db_user
END;
GO
