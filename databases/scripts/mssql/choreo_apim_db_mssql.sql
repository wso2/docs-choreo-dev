-- Create User
IF EXISTS (SELECT name FROM sys.databases WHERE name = N'choreo_apim_db') AND NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = N'choreo_apim_db_user')
BEGIN
    CREATE USER [choreo_apim_db_user] with password = N'${choreo_apim_db_mssql_password}'
...
CREATE TABLE AM_APPLICATION (
    ...
    NAME VARCHAR(150) COLLATE Latin1_General_CS_AS,
    ...
    CONSUMER_TYPE VARCHAR(50) DEFAULT 'EXTERNAL',
...
CREATE TABLE AM_API (
    ...
    VERSION_COMPARABLE VARCHAR(15) DEFAULT DATEDIFF_BIG(MILLISECOND,'1970-01-01 00:00:00.000', SYSUTCDATETIME()),
...
IF NOT  EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[AM_API_BACKEND]') AND TYPE IN (N'U'))

CREATE TABLE AM_API_BACKEND (
    BACKEND_ID VARCHAR(40),
    BACKEND_NAME VARCHAR(255) NOT NULL,
    ENDPOINT_CONFIG VARBINARY(MAX),
    BACKEND_DEFINITION VARBINARY(MAX),
    API_ID  INTEGER NOT NULL,
    CREATED_BY VARCHAR(100),
    CREATED_TIME DATETIME,
    UPDATED_BY VARCHAR(100),
...
IF NOT  EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[AM_API_BACKEND_OPERATION_MAPPING]') AND TYPE IN (N'U'))

CREATE TABLE AM_API_BACKEND_OPERATION_MAPPING (
    ...
    FOREIGN KEY (BACKEND_ID) REFERENCES AM_API_BACKEND (BACKEND_ID) ON DELETE CASCADE,
...
CREATE TABLE AM_APPLICATION_KEY_MAPPING (
    ...
    CHOREO_ENVIRONMENT_ID VARCHAR(100) DEFAULT 'ALL',
    SECURITY_SCHEMA VARCHAR(100) DEFAULT 'OAuth2' NOT NULL,
    APP_INFO VARBINARY(MAX) DEFAULT NULL,
    FOREIGN KEY(APPLICATION_ID) REFERENCES AM_APPLICATION(APPLICATION_ID) ON UPDATE CASCADE ON DELETE CASCADE,
    PRIMARY KEY(APPLICATION_ID,KEY_TYPE,KEY_MANAGER,CHOREO_ENVIRONMENT_ID,SECURITY_SCHEMA)
);
...
CREATE TABLE AM_APPLICATION_ATTRIBUTES (
  ...
  NAME VARCHAR(255) NOT NULL,
  TENANT_ID INTEGER NOT NULL,
  APP_ATTRIBUTE VARCHAR(1024) NULL,
...
CREATE TABLE AM_DEPLOYED_REVISION (
    ...
    VHOST VARCHAR(255) NULL,
    REVISION_UUID VARCHAR(255) NOT NULL,
    DEPLOYED_TIME DATETIME DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (NAME, REVISION_UUID),
    FOREIGN KEY (REVISION_UUID) REFERENCES AM_REVISION(REVISION_UUID) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE AM_GATEWAY_ENVIRONMENT (
  ...
  CHOREO_IS_CRITICAL BIT DEFAULT NULL,
  CHOREO_ENV_TEMPLATE_ID VARCHAR(45) NULL,
  CHOREO_IS_SANDBOX BIT DEFAULT NULL,
  UNIQUE (NAME, ORGANIZATION),
  UNIQUE (UUID),
  PRIMARY KEY (ID)
);
...
-- Choreo specific API related fields --
...
CREATE TABLE CHOREO_AM_API (
    ...
    CHOREO_PROJECT_UUID VARCHAR(45),
    CHOREO_COMPONENT_UUID VARCHAR(45),
    CHOREO_COMPONENT_VERSION_UUID VARCHAR(45),
    FOREIGN KEY (API_UUID) REFERENCES AM_API(API_UUID) ON DELETE CASCADE,
    PRIMARY KEY(API_UUID)
);

-- Choreo specific API Revision related fields --
...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[CHOREO_ENV_AM_ENV_MAPPING]') AND TYPE IN (N'U'))
CREATE TABLE CHOREO_ENV_AM_ENV_MAPPING (
    AM_GW_ENVIRONMENT_ID VARCHAR(45) NOT NULL, -- AM_GW_ENVIRONMENT_UUID or name of the Read Only Environment
    ORGANIZATION VARCHAR(45) NOT NULL,
    CHOREO_PROJECT_UUID VARCHAR(45) NOT NULL,
    CHOREO_ENVIRONMENT_UUID VARCHAR(45) NOT NULL,
    -- FOREIGN KEY(AM_GW_ENVIRONMENT_UUID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID) ON DELETE CASCADE,
    --   Foreign key constraint is not added as 'AM_GW_ENVIRONMENT_UUID' could be from a Read Only Environment
    --   Deleting an entry from this table is done when deleting a dynamic environment
    PRIMARY KEY (AM_GW_ENVIRONMENT_ID, ORGANIZATION, CHOREO_PROJECT_UUID)
);

IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[CHOREO_INTERNAL_IDP_CONFIG]') AND TYPE IN (N'U'))
CREATE TABLE CHOREO_INTERNAL_IDP_CONFIG (
    ORG_ID VARCHAR(50) NOT NULL,
    KM_ID VARCHAR(50) NOT NULL,
    ENV_ID VARCHAR(50) NOT NULL,
    DEV_PORTAL_APP_ENV VARCHAR(20) NULL,
    PRIMARY KEY (ORG_ID, KM_ID, ENV_ID),
    FOREIGN KEY (KM_ID) REFERENCES AM_KEY_MANAGER(UUID) ON DELETE CASCADE
);

IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[CHOREO_DEVPORTAL_ENV_CONFIG]') AND TYPE IN (N'U'))
CREATE TABLE CHOREO_DEVPORTAL_ENV_CONFIG (
    ENV_ID VARCHAR(50) NOT NULL,
    ORG_ID VARCHAR(50) NOT NULL,
    PRIMARY KEY (ENV_ID, ORG_ID)
);

IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[CHOREO_ASB_NAMESPACE]') AND TYPE IN (N'U'))
CREATE TABLE CHOREO_ASB_NAMESPACE (
    ID INTEGER NOT NULL,
    NAME varchar(255) NOT NULL,
...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[CHOREO_ASB_TOPIC]') AND TYPE IN (N'U'))
CREATE TABLE CHOREO_ASB_TOPIC (
    ORGANIZATION VARCHAR(255) NOT NULL,
    DATAPLANE_ID VARCHAR(255) NOT NULL,
    NAMESPACE_ID INT NULL,
    CREATED_AT DATETIME NULL,
    UPDATED_AT DATETIME NULL,
    TYPE VARCHAR(50) NOT NULL,
    PRIMARY KEY (DATAPLANE_ID, TYPE),
    FOREIGN KEY (NAMESPACE_ID) REFERENCES CHOREO_ASB_NAMESPACE(ID) ON DELETE CASCADE,
    CONSTRAINT CHK_TYPE CHECK (TYPE IN ('tokenrevocation', 'notification'))
);

IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[AM_USER_EMAIL]') AND TYPE IN (N'U'))
CREATE TABLE AM_USER_EMAIL (
	EMAIL_ID int IDENTITY(0,1) NOT NULL,
	EMAIL_ADDRESS varchar(150) NOT NULL,
	UNIQUE (EMAIL_ADDRESS),
	PRIMARY KEY (EMAIL_ID)
);

IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[AM_APPLICATION_USER_SHARING]') AND TYPE IN (N'U'))
CREATE TABLE AM_APPLICATION_USER_SHARING (
	APPLICATION_ID varchar(256)  NOT NULL,
	USER_EMAIL_REF_ID int NOT NULL,
	CREATED_AT DATETIME NOT NULL DEFAULT GETDATE(),
	FOREIGN KEY (USER_EMAIL_REF_ID) REFERENCES AM_USER_EMAIL (EMAIL_ID),
	FOREIGN KEY (APPLICATION_ID) REFERENCES AM_APPLICATION(UUID) ON DELETE CASCADE,
	PRIMARY KEY (APPLICATION_ID, USER_EMAIL_REF_ID)
);
...
CREATE INDEX IDX_AAI_OAN ON AM_API (ORGANIZATION, API_NAME) INCLUDE (CONTEXT, CONTEXT_TEMPLATE);
CREATE INDEX IDX_AAOP_PUAU ON AM_API_OPERATION_POLICY (POLICY_UUID, API_UUID) INCLUDE (CLONED_POLICY_UUID, REVISION_UUID);
CREATE INDEX IDX_AAOPM_UMI ON AM_API_OPERATION_POLICY_MAPPING (URL_MAPPING_ID) INCLUDE (DIRECTION, PARAMETERS, POLICY_ORDER, POLICY_UUID);
CREATE INDEX IDX_AKM_OETT ON AM_KEY_MANAGER (ORGANIZATION, ENABLED, TOKEN_TYPE);
CREATE INDEX IDX_IOP_CKTIPK ON IDN_OIDC_PROPERTY (CONSUMER_KEY, TENANT_ID, PROPERTY_KEY) INCLUDE (PROPERTY_VALUE);
CREATE INDEX IDX_SIA_IAKIATTI ON SP_INBOUND_AUTH (INBOUND_AUTH_KEY, INBOUND_AUTH_TYPE, TENANT_ID);
CREATE INDEX IDX_AKM_ERI on AM_KEY_MANAGER (EXTERNAL_REFERENCE_ID);
-- AM_REVISION --
CREATE INDEX IDX_API_UUID ON AM_REVISION (API_UUID);

-- AM_API_LC_EVENT --
CREATE INDEX IDX_API_ID ON AM_API_LC_EVENT (API_ID) INCLUDE (EVENT_DATE, NEW_STATE, PREVIOUS_STATE, USER_ID);

-- AM_DEPLOYED_REVISION --
CREATE INDEX IDX_REVISION_UUID ON AM_DEPLOYED_REVISION (REVISION_UUID) INCLUDE (DEPLOYED_TIME, VHOST);

-- AM_GW_API_DEPLOYMENTS --
CREATE INDEX IDX_LABEL_API_ID ON AM_GW_API_DEPLOYMENTS (LABEL, API_ID) INCLUDE (VHOST);

-- AM_API_REVISION_METADATA --
CREATE INDEX IDX_AARM_RU ON AM_API_REVISION_METADATA (REVISION_UUID) INCLUDE (SCOPE_PREFIX);

-- AM_API_RESOURCE_SCOPE_MAPPING --
CREATE INDEX IDX_AARSM_UMI_TI ON AM_API_RESOURCE_SCOPE_MAPPING (URL_MAPPING_ID, TENANT_ID);

-- AM_API_URL_MAPPING --
CREATE INDEX IDX_AAUM_AIRU ON AM_API_URL_MAPPING (API_ID, REVISION_UUID, AUTH_SCHEME, HTTP_METHOD, THROTTLING_TIER, URL_PATTERN);

-- AM_DEPLOYMENT_REVISION_MAPPING --
CREATE INDEX IDX_ADRM_RU ON AM_DEPLOYMENT_REVISION_MAPPING (REVISION_UUID) INCLUDE (DEPLOYED_TIME, DISPLAY_ON_DEVPORTAL, VHOST);

-- AM_API_ENVIRONMENT_KEYS --
CREATE INDEX IDX_AAEK_AUEI ON AM_API_ENVIRONMENT_KEYS (API_UUID, ENVIRONMENT_ID);

-- AM_GRAPHQL_COMPLEXITY --
create index IDX_GQLC_AI on AM_GRAPHQL_COMPLEXITY (API_ID);

-- AM_SECURITY_AUDIT_UUID_MAPPING --
create index IDX_ASAU_AI on AM_SECURITY_AUDIT_UUID_MAPPING (API_ID);

-- AM_SUBSCRIPTION --
create index IDX_AMSU_AI on AM_SUBSCRIPTION (API_ID);

-- AM_API_COMMENTS --
create index IDX_AACO_AI on AM_API_COMMENTS (API_ID);

-- AM_API_RATINGS --
create index IDX_AARA_AI on AM_API_RATINGS (API_ID);

-- AM_EXTERNAL_STORES --
create index IDX_AEST_AI on AM_EXTERNAL_STORES (API_ID);

-- AM_API_CLIENT_CERTIFICATE --
create index IDX_AACC_AI on AM_API_CLIENT_CERTIFICATE (API_ID);
create index IDX_AACC_AIRU on AM_API_CLIENT_CERTIFICATE (API_ID, REVISION_UUID);

-- AM_API_SERVICE_MAPPING --
create index IDX_AASM_AI on AM_API_SERVICE_MAPPING (API_ID);

-- AM_API_ENVIRONMENT_KEYS --
create index IDX_AAEK_AUI on AM_API_ENVIRONMENT_KEYS (API_UUID);

-- CHOREO_AM_API --
create index IDX_CHAA_AUI on CHOREO_AM_API (API_UUID);
create index IDX_CAA_PRJ on CHOREO_AM_API (CHOREO_PROJECT_UUID);

-- CHOREO_AM_API_REVISION_METADATA --
create index IDX_CAARM_RUI on CHOREO_AM_API_REVISION_METADATA (REVISION_UUID);

-- AM_GW_API_ARTIFACTS --
create index IDX_AGAA_AI on AM_GW_API_ARTIFACTS (API_ID);

-- CHOREO_ASB_TOPIC --
CREATE INDEX IDX_CAT_ORG ON CHOREO_ASB_TOPIC (ORGANIZATION, TYPE);
