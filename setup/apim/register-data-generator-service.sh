...
TOKEN_ENDPOINT="${APIM_URL}/oauth2/token"
CLIENT_REGISTRATION_ENDPOINT="${APIM_URL}/client-registration/v0.17/register"

PAYLOAD='{"callbackUrl":"http://localhost:9090","clientName":"choreo_data_generator_service","owner":"--APP_OWNER--","grantType":"client_credentials password","saasApp":true}'

## Registering Application
ENCODED_CREDENTIALS=$(echo "$APIM_ADMIN_USERNAME:$APIM_ADMIN_PASSWORD" | tr -d '\040\011\012\015' | base64);
echo "${PAYLOAD}" | sed "s/--APP_OWNER--/${APIM_ADMIN_USERNAME}/g" > temp_client_app_create_payload.json
...
echo "Configure the above two values in data-generator configs"
