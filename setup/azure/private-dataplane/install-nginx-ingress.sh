...
LOADBALANCER_IP_RG=$(az group show --name choreo-"${CUSTOMER_NAME}"-dataplane-"${CLUSTER_ENV}"-network-rg --query "name" --output tsv)

echo "LB IP RG is: ${LOADBALANCER_IP_RG}"

LOADBALANCER_VNET_NAME=$(az network vnet show --name choreo-"${CUSTOMER_NAME}"-dataplane-"${CLUSTER_ENV}"-virtual-network --resource-group "${LOADBALANCER_IP_RG}" --query "name" --output tsv)

echo "LB VNET NAME is: ${LOADBALANCER_VNET_NAME}"

LOADBALANCER_SUBNET_NAME=$(az network vnet subnet show --resource-group "${LOADBALANCER_IP_RG}" --vnet-name "${LOADBALANCER_VNET_NAME}" --name choreo-"${CUSTOMER_NAME}"-dp-"${CLUSTER_ENV}"-loadbalancer-subnet --query "name" --output tsv)
...
if [[ "${CHOREO_ENV}" == "dev" ]]; then
	    CUSTOMDNS_LOADBALANCER_IP=$(az network firewall policy rule-collection-group collection list --policy-name choreo-dev-hub-fw-policy --resource-group choreo-dev-network-rg --rcg-name 'private-dp-dnat-collection-group' --query "[1].rules[?contains(name, 'public-ip-customdns-${CLUSTER_ENV}-http')].translatedAddress" --output tsv)
	
    elif [[ "${CHOREO_ENV}" == "stage" ]]; then
	    #todo; change this to same as dev once the Firewall has been upgraded to premium in Stage
	    CUSTOMDNS_LOADBALANCER_IP=$(az network firewall nat-rule collection show --firewall-name choreo-stg-dp-fw --resource-group choreo-stg-hub-network-rg --collection-name choreo-stg-dnat-rule-collection-http --query "rules[?contains(name, 'public-ip-customdns-${CLUSTER_ENV}-http')].translatedAddress" --output tsv)
    else
	    CUSTOMDNS_LOADBALANCER_IP=$(az network firewall nat-rule collection show --firewall-name choreo-"${CUSTOMER_NAME}"-dp-fw --resource-group choreo-"${CUSTOMER_NAME}"-hub-network-rg --collection-name choreo-"${CUSTOMER_NAME}"-dnat-rule-collection-http --query "rules[?contains(name, 'public-ip-customdns-${CLUSTER_ENV}-http')].translatedAddress" --output tsv)
fi
