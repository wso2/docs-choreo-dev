...
SYSTEM_CSI_KEY_VAULT_CLIENT_SECRET=$(az ad app credential reset --id "${SYSTEM_CSI_KEY_VAULT_CLIENT_ID}" --append --display-name "${CLUSTER_NAME}-choreodp" --years 2 | grep password | cut -d ":" -f2 | cut -d '"' -f 2)
...
SYSTEM_CSI_KEY_VAULT_CLIENT_SECRET=$(az ad app credential reset --id "${SYSTEM_CSI_KEY_VAULT_CLIENT_ID}" --append --display-name "${CLUSTER_NAME}-obs" --years 2 | grep password | cut -d ":" -f2 | cut -d '"' -f 2)
