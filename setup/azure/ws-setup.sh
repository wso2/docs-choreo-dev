...
bash workspace/reloader/configure-reloader.sh

echo "--- Installing Cert Manager..."
bash workspace/cert-manager/configure-cert-manager.sh

echo "--- Installing Emberstack reflector..."
bash workspace/reflector/configure-reflector.sh

echo "--- Creating secrets for DNS-01 challenge..."
bash workspace/lets-encrypt-certs/configure-lets-encrypt-certs.sh

echo "--- Add OMS Agent Config"
bash workspace/oms-agent/configure-oms-agent.sh

echo "--- Configure CSI Secret Store"
bash workspace/secret-store-csi-driver/configure-csi-secret-store.sh

echo "--- Setup Nginx Ingress"
bash workspace/nginx-ingress-controllers/configure-ingress-controllers.sh
