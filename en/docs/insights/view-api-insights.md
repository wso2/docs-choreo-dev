...
!!! Note
    The geo-map page utilizes the incoming request's IP address to derive the geolocation. Since the actual source IP address can be replaced if there are intermediate hosts between the client and the gateway node, the reverse proxy/load balancer fronting the gateway node should add the <b>x-forwarded-for</b> header to each request in order to preserve the source IP address. If this isn't configured correctly, the geo map page will not show any data. For testing purposes, an x-forwarded-for header can be manually added to the request.  
