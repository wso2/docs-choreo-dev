...
To add third-party libraries to the Micro Integrator, create a new directory named `libs` within the root of the Micro Integrator project. It is important to note that this directory should be placed specifically at the project root, rather than the root of the GitHub repository. When you deploy the component, all the JARs in the `libs` directory in the project path will be included in the Micro Integrator runtime.
