...
- **I’m seeing a blank page or a 502 error after I deploy my web application**
    ...
    - Double-check the build output directory, especially if you are using a Dockerfile-less preset. Ensure that the specified output directory matches the actual output directory generated during the build process. For example, if by mistake you have entered `public/` as the output directory when it should have been `build/`.
