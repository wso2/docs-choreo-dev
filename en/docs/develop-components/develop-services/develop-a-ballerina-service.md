...
Now that we have connected the source repository, and configured the endpoint details, it's time to build and deploy the greeter service.
    ...
    !!! note
        If you are rebuilding the Ballerina service component after downgrading the Ballerina version, ensure that before building the code, the version of the Ballerina distribution mentioned in the Ballerina.toml file matches the distribution-version specified in the Dependencies.toml file.
