...
Choreo uses an in-built Identity Provider (IdP) by default to manage OAuth clients and generate tokens required to authenticate Choreo APIs. Choreo also grants admin permission to privileged users to configure a required authorization server as an identity provider(IdP) via the Choreo Console. This allows you to add one or more external IdPs to your Choreo organization to obtain access tokens and exchange with Choreo access tokens to invoke APIs. This capability lets you expose your APIs to users who are external to Choreo.
