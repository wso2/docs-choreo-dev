...
In this step, you will configure Managed Authentication, create a user to access the web application, and then deploy the web application. For more information on Choreo's Managed Authentication refer to the [Managed Authentication](../authentication-and-authorization/secure-web-applications-with-managed-authentication.md) documentation.
...
2. To proceed with creating a user with the populated username and password, click **Create**. Make sure you copy the populated username and password to use when you test the front-end application.
    ...
    !!! tip
         - By default, your test user base will consist of the demo user created here. You can modify the test user base by following the [Configure a User Store with the Built-In Identity Provider](../administer/configure-a-user-store-with-built-in-idp.md) documentation.
