...
{{ product_name }} allows you to share and reuse your services, accelerating development and enhancing efficiency in building integrated applications through connections.
...
You can consume a {{ product_name }}-deployed service within another service. Consuming connections from within {{ product_name }} services is seamless and straightforward. Follow the steps below to consume a {{ product_name }} service:
...
=== "Component.yaml file (v1.1)"
    ...
    2. If you've previously added a `connectionReferences` section under `dependencies`, append this as another item under `connectionReferences`. Upon deploying the component, {{ product_name }} automatically creates a subscription if applicable and the necessary configurations to establish the connection will be injected into the {{ product_name }}-defined environment variables.
          ...
          If you want to use custom environment variable names instead of the {{ product_name }}-defined ones, you must define the environment configurations under the `configuration` section in the same file.
          The following is a sample snippet: 
          ...
          |----------------|------------|---------------------------------------|---------------|--------------|
          | ServiceURL     | string     | Service URL of the {{ product_name }} service     | false         | false        |
          | ConsumerKey    | string     | Consumer key of the {{ product_name }} service    | false         | false        |
          | ConsumerSecret | string     | Consumer secret of the {{ product_name }} service | false         | true         |
          | TokenURL       | string     | Token URL of the STS                  | false         | false        |
          | ChoreoAPIKey   | string     | API key of the {{ product_name }} service         | false         | true         |
...
=== "Component.yaml file (v1.0)"
    ...
    2. Replace `<YOUR_ENV_VARIABLE_NAME_HERE>` with an appropriate environment variable name of your choice. If you have previously added a service reference section under `dependencies`, append this as another item under `serviceReferences`. 
          ...
          Upon deploying the component, {{ product_name }} automatically creates a subscription if applicable and populates the specified environment variables with actual values.
...
To consume a {{ product_name }} service with the visibility level set to organization or public and secured by the OAuth 2.0 security scheme, you must obtain an OAuth 2.0 token from the token endpoint. Subsequently, you can use the token to invoke the service.
...
=== "API key security scheme"
     ...
     To invoke the API, use the `{{ product_name }}-api-key` header with the API key value retrieved from the corresponding environment variable as described in [step 2](#step-2-read-configurations-within-the-application).
     ...
     ``` java
       const response = await axios.get(serviceURL/{RESOURCE_PATH}, {
           headers: {
             '{{ product_name }}-API-Key': `${choreoApiKey}`
           }
       });
     ```
...
=== "OAuth 2.0 security scheme"
    ...
    !!! note
        If you want to consume a {{ product_name }} service at the project visibility level, you don't need to obtain a token. You can directly invoke the service using the resolved URL.
