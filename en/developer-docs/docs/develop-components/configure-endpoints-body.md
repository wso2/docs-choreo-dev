...
{{ product_name }} defines endpoints by combining port binding, protocol, endpoint name, network visibility, endpoint schema, and additional protocol-related fields. The following table describes each attribute of an endpoint.
...
* For `Ballerina` and `WSO2 MI` build presets, {{ product_name }} automatically detects the endpoint details for REST APIs.
...
Exposing endpoints as managed APIs is crucial to ensure secure and controlled access to the services being exposed. When a user wants to expose their written service to the outside world or to the organization at large, there is an inherent security risk involved. To mitigate this risk, the {{ product_name }} platform is built with an internal (access within the organization only) or external (publicly accessible) gateway that is protected with {{ product_name }} API management making the services secure by design.

!!! note
    This feature is not available for gRPC, UDP, and TCP endpoints.

If you want to expose an endpoint as a managed API in {{ product_name }}, you need to set the network visibility to either Organization or Public. This allows the endpoint to be exposed through the {{ product_name }} API Gateway, which provides a number of benefits, including:
...
Once you deploy the service component, {{ product_name }} will expose the endpoint as a managed API through the {{ product_name }} API Gateway. You can then use the full lifecycle API management features provided by {{ product_name }} to test, deploy, maintain, monitor, and manage your API using the API management features.
...
If a component has multiple endpoints, {{ product_name }} allows you to change the endpoint corresponding to the default component URL. For a component with a single endpoint, the default URL automatically corresponds to that endpoint.
...
!!! note
     ...
     - {{ product_name }} does not allow you to change the default endpoint of a component if you create multiple deployment tracks within its current major version, or promote the component beyond its initial environment.
