...
{{ product_name }} is an Internal Developer Platform (IDevP) that makes it easy to deploy, monitor, and manage your cloud-native services. This allows you to focus on innovation and implementation.

With {{ product_name }}, you can deploy services written in your preferred programming language in just a few simple steps.

In this guide, you will:

- Use a pre-built service that manages a book list.
- Build and deploy the service in {{ product_name }} using the `Node.js` build preset. The service runs on port 8080.
- Test the service.

For a video tutorial, check out [Deploy Your First Service with {{ product_name }}](https://www.youtube.com/watch?v=-qoweQWCiYM).

## Prerequisites

1. GitHub account: Fork the [{{ product_name }} sample book list service repository](https://github.com/wso2/choreo-sample-book-list-service/), which contains the sample for this guide.
...
!!! note
    The file paths are relative to `<{{ product_name }}-sample-book-list-service>/`.
| Filepath               | Description                                                                   |
|------------------------|-------------------------------------------------------------------------------|
| `app.mjs`              | The Node.js (JavaScript) service code.                                        |
| `.choreo/component.yaml` | {{ product_name }}-specific configuration for exposing the service.                       |
...
{{ product_name }} creates a default project for you when you create an organization. You can use that project for this guide and go straight to step 2. Follow the steps below only if you want to work in a separate project:
...
2. Click **Authorize with GitHub** to connect {{ product_name }} to your GitHub account. If you haven't connected your GitHub repository to {{ product_name }}, enter your credentials and select the repository you forked earlier to install the [{{ product_name }} GitHub App](https://github.com/marketplace/choreo-apps).
...
3. Enter the following information:
    ...
    | **Repository**        | {{ product_name }}-sample-book-list-service  |
    | **Branch**            | main                             |
    | **Component Directory** | /                 |
...
After successfully testing your service, explore other {{ product_name }} features like [managing](../api-management/lifecycle-management.md), [observing](../monitoring-and-insights/observability-overview.md), and [DevOps](../devops-and-ci-cd/view-runtime-details.md).
