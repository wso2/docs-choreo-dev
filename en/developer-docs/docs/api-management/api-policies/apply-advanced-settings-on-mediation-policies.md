...
| **Max Header Size**         | Determines the maximum header size (in bytes) allowd by the mediation application. By default, allowed max header size is 8192b. | Set the `maxHeaderSize` parameter in the **Advanced Settings** field. <br> ![Detailed access log setting](../../assets/img/api-management/api-policies/advanced-settings/max-header-size-setting.png)  | The mediation service can accept the headers up to the configured size. Any request with a header larger than the provided value would be served with 431 HTTP status code.    |
