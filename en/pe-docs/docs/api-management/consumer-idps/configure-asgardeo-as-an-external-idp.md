...
Asgardeo is an identity-as-a-service (IDaaS) solution designed to create seamless login experiences for your applications. Asgardeo seamlessly integrates with {{ product_name }}, providing powerful API access control through the use of API scopes. This enables restricting API access to designated user groups. By configuring Asgardeo as an external IdP in {{ product_name }}, you can leverage your Asgardeo user stores to manage API access control effectively. This guide walks you through the steps to set up Asgardeo as your external IdP.

!!! note
    These identity provider settings are for consuming the components deployed within your organization, not for authenticating developers who signing in to the {{ product_name }} Console.
## Prerequisites
...
## Add Asgardeo as an external IdP in {{ product_name }}

Follow the steps below to add Asgardeo as an external IdP in {{ product_name }}:
...
Now you have configured Asgardeo as an external IdP in {{ product_name }}.
