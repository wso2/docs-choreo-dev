...
{{ product_name }} provides the scale-to-zero capability for HTTP applications you deploy in the data plane. This lets you run your components in a serverless mode.
...
## How Scale to Zero works in {{ product_name }}

!!! info
    For service components and web-apps you create after February 23, 2024, {{ product_name }} enables the scale-to-zero feature by default.
...
When Scale to Zero is enabled, you can set the maximum number of replicas for deployments with this capability. {{ product_name }} dynamically scales deployments up to meet high HTTP traffic demand, up to the specified number of replicas. If the pending requests surpass the defined threshold under **Number of pending requests to spawn a new pod**, {{ product_name }} automatically adds a new replica to handle the increased load.

![Free User - Scale to Zero](../../assets/img/devops-and-ci-cd/scaling/scale-to-zero-view.png){.cInlineImage-full}

## Enable scale to zero

For service components and web-apps you create after February 23, 2024, {{ product_name }} enables the scale-to-zero feature by default. When deploying or promoting the component, the deployment will automatically scale-to-zero.
Upon the next request to the deployed service, a replica will be created to serve the request.

!!! note  
    - For the services which contain at least one endpoint with the network visibility as **Project**, {{ product_name }} will not automatically scale-to-zero those components when you deploy or promote them.
...
You can independently scale {{ product_name }} components in both the **Development** and **Production** environments. The deployment card indicates the scaling status of each environment. To configure the scale-to-zero feature for a specific environment, click on the **scale-to-zero** link, which redirects to the **K8s Operations** → **Scaling** page.
