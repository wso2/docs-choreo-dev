...
!!! Note -
      Once the database is created, you can add it to the marketplace so developers can discover these databases and use them in their applications. For more details, see [Add {{ product_name }}-Managed Databases and Caches to the Marketplace.](../add-{{ product_name }}-managed-databases-and-caches-to-the-marketplace) 
