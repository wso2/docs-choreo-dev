...
In {{ product_name }}, you can manage egress traffic originating from your applications by setting up an allow list or deny list. By default, egress traffic is allowed to any destination unless specifically restricted.
...
5.  Select the type of egress control to apply and add the required rules:
    ...
    !!! note
             ...
             - If you use the **Deny All** type, be sure to add {{ product_name }}-managed database hosts to the allowed list.
...
1. Sign in to the [{{ product_name }} Console](https://console.choreo.dev/).
2. In the {{ product_name }} Console header, go to the **Project** list and select your project.
3.  In the left navigation menu, click **Governance**, then click **Egress Control**.
